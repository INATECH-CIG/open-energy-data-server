...
def perform_decomposition_analysis(config: PipelineConfig, gen_dfs=None, comm_dfs=None):
    ...
    for bz in comm_dfs:
        ...
        if not raw_total_imports.empty:
            ...
            df_to_timescale(raw_total_imports, f"{bz}_import_comm_flow_total_per_bidding_zone", config.db_schema_name)
        ...
        if not raw_netted_imports.empty:
            ...
            df_to_timescale(raw_netted_imports, f"{bz}_import_comm_flow_total_netted_per_bidding_zone", config.db_schema_name)
    ...
    for i, bz in enumerate(valid_zones):
        ...
        if total_imp_list:
            ...
            df_to_timescale(total_full, f"{bz}_import_comm_flow_total_per_type_per_bidding_zone", config.db_schema_name)
            df_to_timescale(netted_full, f"{bz}_import_comm_flow_total_netted_per_type_per_bidding_zone", config.db_schema_name)
            ...
            df_to_timescale(per_type, f"{bz}_import_comm_flow_total_per_type", config.db_schema_name)
            df_to_timescale(per_type_net, f"{bz}_import_comm_flow_total_netted_per_type", config.db_schema_name)
            ...
            df_to_timescale(per_agg, f"{bz}_import_comm_flow_total_per_agg_type", config.db_schema_name)
            df_to_timescale(per_agg_net, f"{bz}_import_comm_flow_total_netted_per_agg_type", config.db_schema_name)
...
def _decompose_and_save(config, traced_dfs, base_dir, label, gen_dfs):
    ...
    for bz in config.zones:
        ...
        df_to_timescale(traced_dfs[bz], f"{bz}_import_flow_tracing_{label}_per_bidding_zone", config.db_schema_name)
        ...
        if type_dfs:
            ...
            df_to_timescale(full_type, f"{bz}_import_flow_tracing_{label}_per_type_per_bidding_zone", config.db_schema_name)
            ...
            df_to_timescale(per_type, f"{bz}_import_flow_tracing_{label}_per_type", config.db_schema_name)
            ...
            df_to_timescale(per_agg, f"{bz}_import_flow_tracing_{label}_per_agg_type", config.db_schema_name)
...
def perform_pooling_analysis(config: PipelineConfig, gen_dfs=None, comm_dfs=None, phys_flow_dfs=None):
    ...
    def save_pool(pooled_dict, name, file_p):
        ...
        for bz, df_imp in pooled_dict.items():
            ...
            df_to_timescale(df_imp, f"{bz}_pooled_{file_p}_per_bidding_zone", config.db_schema_name)
            ...
            if type_dfs:
                ...
                df_to_timescale(full, f"{bz}_pooled_{file_p}_per_type_per_bidding_zone", config.db_schema_name)
                ...
                df_to_timescale(per_type, f"{bz}_pooled_{file_p}_per_type", config.db_schema_name)
                ...
                df_to_timescale(per_agg,f"{bz}_pooled_{file_p}_per_agg_type", config.db_schema_name)
    # ---------------------------------------------------------
...
def perform_post_processing_aggregation(config: PipelineConfig):
    ...
    for bz in config.zones:
        ...
        df_to_timescale(res_imp_bz, f"{bz}_annual_totals_import_per_bidding_zone_{year}", config.db_schema_name)
        df_to_timescale(res_exp_bz, f"{bz}_annual_totals_export_per_bidding_zone_{year}", config.db_schema_name)
        df_to_timescale(res_imp_type, f"{bz}_annual_totals_import_per_type_{year}", config.db_schema_name)
        df_to_timescale(res_imp_type, f"{bz}_annual_totals_export_per_type_{year}", config.db_schema_name)
        ...
        df_to_timescale(res_agg, f"{bz}_annual_totals_import_per_agg_type_{year}", config.db_schema_name)
